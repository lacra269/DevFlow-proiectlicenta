...
export default function AddPost() {
    ...
    const handlePostSubmit = async () => {
        // ... (aceeași logică de validare și fetch ca înainte) ...
    ...
    // --- Afișare dacă nu e logat (stil compact) ---
    if (!session) {
        return (
    ...
    return (
        // Container: Gradient, padding redus, umbră dinamică
        <div className={`p-2 sm:p-3 rounded-xl bg-gradient-to-br from-purple-50 via-blue-50 to-white mb-4 transition-all duration-300 ease-in-out ${isFocused ? 'shadow-md' : 'shadow-sm'}`}>
            <div className="flex items-start space-x-2 sm:space-x-3">
                {/* Avatar: Mai mic, fără efecte extra */}
                <Image
                    src={session.user?.image || "https://placehold.co/36x36/cccccc/ffffff?text=?"}
                    alt="User Avatar"
                    className="w-8 h-8 sm:w-9 sm:h-9 object-cover rounded-full flex-shrink-0 mt-0.5" // Redus dimensiunea și ajustat mt
                    width={36} height={36}
                    onError={(e) => (e.currentTarget.src = "https://placehold.co/36x36/cccccc/ffffff?text=?")}
                />
                {/* Zona de input și butoane */}
                <div className="flex-1 min-w-0">
                    {/* Textarea: Text mai mic, expansiune limitată */}
                    <textarea
                        ...
                        rows={isFocused || postText || file ? 2 : 1} // Expansiune limitată la 2 rânduri
                        disabled={isPosting}
                    />
                    ...
                    {/* Preview fișier: Mai compact */}
                    {localPreviewUrl && file && (
                        <div className="mt-2 mb-1.5 relative inline-block bg-gray-100/70 p-0.5 rounded-md shadow-sm max-w-[120px]"> {/* max-w redus */}
                            {file.type.startsWith("image") ? (
                                <Image src={localPreviewUrl} alt="Preview" className="max-h-16 w-auto object-contain rounded" width={100} height={64} /> /* max-h redus */
                            ) : file.type.startsWith("video") ? (
                                <video controls className="max-h-16 w-full object-contain rounded bg-black" src={localPreviewUrl} /> /* max-h redus */
                            ...
                            <button
                                ...
                                disabled={isPosting}
                                style={{ width: '14px', height: '14px' }} // Buton X mai mic
                            >
                    ...
                    {/* Butoane acțiuni: Mai compacte */}
                    <div className="flex items-center justify-between mt-1.5"> {/* mt redus */}
                        {/* Buton Media: Iconiță mai mică, padding redus */}
                        <label htmlFor="file-upload-compact" className={`text-purple-600 hover:text-purple-800 cursor-pointer p-1 rounded-full hover:bg-purple-100/70 transition-colors duration-200 ${isPosting ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            <Paperclip size={16} /> {/* Iconiță mai mică */}
                        </label>
                        <input type="file" accept="image/*,video/*" onChange={handleFileChange} className="hidden" id="file-upload-compact" disabled={isPosting}/>

                        {/* Buton Postează: Gradient, text/iconiță mici, padding redus */}
                        <button
                            onClick={handlePostSubmit}
                            className={`bg-gradient-to-r from-purple-500 to-blue-500 text-white py-1 px-3 rounded-full hover:opacity-95 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-1 focus:ring-offset-purple-50 disabled:opacity-60 disabled:cursor-not-allowed text-xs font-medium transition-all duration-200 flex items-center shadow-sm hover:shadow-md`} // py/px reduse
                            disabled={isPosting || (!postText.trim() && !file)}
                        >
                            {isPosting ? (
                                <>
                                    <div className="w-3 h-3 border-t-2 border-b-2 border-white rounded-full animate-spin mr-1.5"></div> {/* spinner mic */}
                                    <span>...</span>
                                </>
                             ) : (
                                <>
                                    <Send size={12} className="mr-1"/> {/* Iconiță mai mică */}
                                    <span>Postează</span>
                                </>
                             )}
                        </button>
